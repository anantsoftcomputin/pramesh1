...
const LeaderCard = ({ leader, isActive, onClick }) => {
  return (
    <div 
      className={`bg-white rounded-lg shadow-soft overflow-hidden cursor-pointer transition-all duration-300 ${isActive ? 'ring-2 ring-primary-500' : ''}`}
      onClick={onClick}
    >
      <div className="p-6">
        <h3 className="text-xl font-semibold mb-2 text-primary-700">{leader.name}</h3>
...
const LeaderDetails = ({ leader }) => {
  return (
    <div className="bg-white rounded-lg shadow-soft p-6 mt-6">
      <h3 className="text-xl font-semibold mb-4 text-primary-700">{leader.name} - Detailed Profile</h3>
...
const LeadershipTeam = () => {
  ...
  return (
    <section className="mb-12">
      <h2 className="text-2xl font-bold mb-6 text-primary-800">Our Leadership Team</h2>
