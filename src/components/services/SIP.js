...
import { IndianRupee, Calendar, TrendingUp, HelpCircle, ArrowRight } from 'lucide-react';
const SIP = () => {
  ...
  return (
    <section className="mb-12">
      <h2 className="text-4xl font-bold mb-6 text-center gradient-text">Systematic Investment Plans (SIPs)</h2>
      <p className="mb-8 text-lg text-primary-700 text-center max-w-3xl mx-auto">
        A Systematic Investment Plan (SIP) is a disciplined investment approach where you invest a fixed amount at regular intervals into mutual funds. It's like planting a seed today and watching it grow into a lush tree over time.
      </p>
      
      <div className="bg-primary-50 p-8 rounded-2xl shadow-soft mb-12">
        <h3 className="text-2xl font-semibold mb-4 text-primary-800">Our SIP Offerings:</h3>
        <motion.ul 
          className="space-y-4"
        ...
        >
          {[
            { title: "Dream Builder SIP", description: "Perfect for long-term goals like buying a home or funding education." },
            { title: "Wealth Accumulator SIP", description: "Ideal for building substantial wealth over time with higher risk appetite." },
            { title: "Retirement Bliss SIP", description: "Designed to secure your golden years with a steady growth plan." },
            { title: "Tax Saver SIP", description: "Combine wealth creation with tax benefits under Section 80C." },
            { title: "Child's Future SIP", description: "Invest in your child's aspirations and ensure they have the financial support they need." }
          ].map((item, index) => (
            <motion.li key={index} variants={itemVariants} className="flex items-start">
              <ArrowRight className="w-5 h-5 text-primary-500 mr-2 mt-1 flex-shrink-0" />
              <div>
                <strong className="text-primary-700">{item.title}:</strong> {item.description}
              </div>
            </motion.li>
          ))}
        </motion.ul>
