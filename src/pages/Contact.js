  ...
  return (
    <form className="bg-white p-6 rounded-lg shadow-soft">
      ...
      <div className="space-y-4">
        <input type="text" placeholder="Name" className="w-full p-2 border border-primary-200 rounded" />
        <input type="tel" placeholder="Phone" className="w-full p-2 border border-primary-200 rounded" />
        <input type="email" placeholder="Email" className="w-full p-2 border border-primary-200 rounded" />
        ...
        <textarea placeholder="Your Message" rows="4" className="w-full p-2 border border-primary-200 rounded"></textarea>
        <button type="submit" className="w-full bg-primary-500 text-white py-2 rounded hover:bg-primary-600 transition duration-300">Send Message</button>
      </div>
    </form>
  );
};
...
const Contact = () => {
  ...
  const branches = [
    { name: "AKOTA", address: "F-13, Om park Society Opp. Kalash Circle, Near White Potato restaurant, Akota, Vadodara -390012" },
    { name: "MANJALPUR", address: "112-Ronak Plaza, Nr. Tulshidham Char Rasta, Manjalpur , Vadodara-390011" },
    { name: "GANGOTRI", address: "5, swastik apartment, Near Gangotri Apartment, R V desai road, Vadodara – 390001" },
    { name: "WAGHODIA ROAD", address: "FF-D-6 Gajanan park Co-op Hsg Soc., Opp Indrapuri Atithi Gruh, Waghodia road, Vadodara -390019" },
  ];

  return (
    <div className="bg-gradient-to-b from-primary-50 to-white min-h-screen">
      <div className="container mx-auto px-4 py-16">
        <h1 className="text-4xl md:text-5xl font-bold mb-12 text-center gradient-text">Contact Us</h1>
          ...
          <div className="space-y-6">
            <ContactInfo title="Main Office">
              <div className="flex items-start mb-4">
                ...
                <p>Pramesh Wealth Pvt. Ltd. "Pramesh House" 8, Vedant Bunglow, Main Kalali road, Kalali, Vadodara- 390012</p>
              </div>
              ...
              {branches.map((branch, index) => (
                <div key={branch.name} className="mb-2">
                  ...
                  >
                    ...
                    <ChevronDown className={`w-5 h-5 transform transition-transform duration-300 ${activeBranch === index ? 'rotate-180' : ''}`} />
                  </button>
                  {activeBranch === index && <BranchInfo name={branch.name} address={branch.address} />}
                </div>
              ))}
            </ContactInfo>
          </div>
        ...
        <div className="mt-12 bg-white p-6 rounded-lg shadow-soft">
          <h2 className="text-2xl font-semibold mb-4 text-primary-700">Location Map</h2>
