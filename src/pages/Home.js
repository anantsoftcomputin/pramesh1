...
import { ArrowRight, BarChart2, Shield, Users, TrendingUp, PiggyBank, Clock, Calculator, IndianRupee,Mail, Phone, MapPin } from 'lucide-react';
...
const Hero = ({ title, subtitle, ctaText, ctaLink }) => {
  ...
  return (
    <div className="relative overflow-hidden h-[50vh] md:h-[60vh]"> {/* Adjusted height */}
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            ...
            >
              ...
              >
                {ctaText}
                <ArrowRight className="ml-2 w-4 h-4" /> {/* Adjusted icon size */}
              </Link>
            </motion.div>
          </div>
        </div>
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setStockData(prevData => 
        prevData.map(stock => ({
          ...stock,
          price: (parseFloat(stock.price.replace(',', '')) + (Math.random() - 0.5) * 10).toFixed(2),
      ...
      <div className="flex animate-ticker">
        {stockData.concat(stockData).map((stock, index) => (
          <div key={index} className="flex items-center mx-4 whitespace-nowrap">
            ...
            <span className={`ml-2 ${stock.change.startsWith('-') ? 'text-red-400' : 'text-green-400'}`}>
              {stock.change}
            </span>
          </div>
        ))}
...
const MutualFundCalculator = () => {
  ...
  return (
    <div className="bg-primary-50 p-6 rounded-xl shadow-soft">
      ...
      <InputField label="Monthly Investment" value={investment} onChange={setInvestment} min={500} max={1000000} step={500} />
      <InputField label="Investment Period (Years)" value={years} onChange={setYears} min={1} max={40} step={1} />
      <InputField label="Expected Return Rate (%)" value={returnRate} onChange={setReturnRate} min={1} max={30} step={0.5} />
...
const FixedDepositCalculator = () => {
  ...
  return (
    <div className="bg-primary-50 p-6 rounded-xl shadow-soft">
      ...
      <InputField label="Principal Amount" value={principal} onChange={setPrincipal} min={1000} max={10000000} step={1000} />
      <InputField label="Interest Rate (%)" value={rate} onChange={setRate} min={1} max={15} step={0.1} />
      <InputField label="Tenure (Years)" value={tenure} onChange={setTenure} min={1} max={20} step={1} />
...
const RetirementCalculator = () => {
  ...
  return (
    <div className="bg-primary-50 p-6 rounded-xl shadow-soft">
      ...
      <InputField label="Current Age" value={currentAge} onChange={setCurrentAge} min={18} max={100} step={1} />
      <InputField label="Retirement Age" value={retirementAge} onChange={setRetirementAge} min={45} max={100} step={1} />
      <InputField label="Monthly Expense" value={monthlyExpense} onChange={setMonthlyExpense} min={10000} max={1000000} step={1000} />
      <InputField label="Expected Inflation (%)" value={inflation} onChange={setInflation} min={1} max={15} step={0.5} />
...
const TaxSavingCalculator = () => {
  ...
  return (
    <div className="bg-primary-50 p-6 rounded-xl shadow-soft">
      ...
      <InputField label="Annual Income" value={income} onChange={setIncome} min={250000} max={10000000} step={10000} />
      <InputField label="Section 80C Investment" value={investment} onChange={(value) => setInvestment(Math.min(value, 150000))} min={0} max={150000} step={1000} />
...
const HomePage = () => {
  ...
  const testimonials = [
    { name: "Ananya Sharma", text: "Working with [Your Company Name] has been a game-changer. Their personalized approach helped me clarify my goals and set me on the path to financial freedom." },
    { name: "Rajesh Patel", text: "I've seen my investments grow steadily over the years. Their expert advice and diverse portfolio options have truly made a difference." },
    { name: "Priya Mehta", text: "The team's dedication to understanding my unique financial situation has been impressive. I feel confident about my financial future." },
    { name: "Vikram Singh", text: "The wealth management strategies provided by the team have exceeded my expectations. I've achieved financial milestones I never thought possible." },
    { name: "Neha Gupta", text: "Their innovative approach to financial planning has opened up new investment avenues for me. I'm grateful for their expertise and guidance." }
  ];
  ...
  <h2 className="text-3xl font-bold text-center text-primary-900 mb-8">Our Services</h2>
...
{/* News Ticker */}
      ...
      <section className="py-16 px-4 bg-primary-50">
        <h2 className="text-3xl font-bold text-center text-primary-900 mb-12">Financial Calculators</h2>
        <div className="max-w-4xl mx-auto">
            ...
            {[
              ...
              <motion.button
                ...
                className={`px-4 py-2 rounded-full flex items-center ${
                  activeCalculator === id ? 'bg-primary-500 text-white' : 'bg-white text-primary-600'
                }`}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeCalculator === 'fixedDeposits' && <FixedDepositCalculator />}
      ...
      <section className="py-16 px-4 bg-secondary-50">
        <h2 className="text-3xl font-bold text-center text-secondary-900 mb-12">What Our Clients Say</h2>
      ...
      <section className="py-20 px-4 text-center bg-primary-500 text-white">
        ...
        <p className="text-xl mb-8">Schedule a free consultation with our experts</p>
        <Link 
          to="/contact" 
      ...
      <footer className="py-8 px-4 text-center text-sm text-neutral-600">
        ...
        <p className="mt-2">
          © 2024 Pramesh Wealth. All rights reserved. SEBI Registration No: XXXXXXXXXX
        </p>
      </footer>
    </div>
  );
